...
def main():
    ...
    # Anno dir
    new_run_parser.add_argument('--anno-dir', '-a', type=str,
                                help='The directory where the annotations are stored. Specify only if you have annotations in a different folder.')

    new_run_parser.add_argument(
        '--batch-size', '-b', required=True, type=int, help='The batch size.')
    new_run_parser.add_argument('--epochs', '-e', default=300,
    ...
    new_run_parser.add_argument(
        '--message', '-m', default=256, type=int, help='The length in bits of the watermark.')
    ...
    new_run_parser.add_argument('--vocab-path', '-v', type=str, default='./data/vocab.pkl', help='load the vocab')


    continue_parser = subparsers.add_parser(
        'continue', help='Continue a previous run')
    ...
    # Anno dir
    continue_parser.add_argument('--anno-dir', '-a', required=False, type=str,
                                 help='The directory where the annotations are stored. Specify a value only if you want to override the previous value.')
    ...
    if args.command == 'continue':
        ...
        options_file = os.path.join(
            this_run_folder, 'options-and-config.pickle')
    ...
    train(model, device, hidden_config,
          train_options, this_run_folder, tb_logger, vocab)
